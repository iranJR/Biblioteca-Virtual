...
if(!empty($_GET['busca'])) {
    $sql = "SELECT * FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario AND emprestimo.situacao AND usuario.idUsuario = :id AND livro.titulo like :busca LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
    $statement = $pdo->prepare($sql);
    $statement->bindValue(':busca', '%' . $busca . '%');
    $statement->bindValue(':id', $idUsuario);
}else{
    $sql = "SELECT * FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario AND usuario.idUsuario = :id ORDER BY emprestimo.situacao LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
    $statement = $pdo->prepare($sql);
    $statement->bindValue(':id', $idUsuario);
}
$statement->execute();
...
if(!empty($_GET['busca'])) {
    $sqlContador = "SELECT COUNT(*) AS total_registros FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario AND usuario.idUsuario = :id AND livro.titulo like :busca";
    $statement = $pdo->prepare($sqlContador);
    $statement->bindValue(':busca', '%' . $busca . '%');
    $statement->bindValue(':id', $idUsuario);
}else{
    $sqlContador = "SELECT COUNT(*) AS total_registros FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario AND usuario.idUsuario = :id";
    $statement = $pdo->prepare($sqlContador);
    $statement->bindValue(':id', $idUsuario);
}
$statement->execute();
                        ...
                        <div class="container col-md-8">
                            <div class="form-row">
                                <div id="buscarRegistro" class='input-group'>
                                    <input id='busca' name="busca" type='text' class='form-control' placeholder='Buscar emprestimo...'>
