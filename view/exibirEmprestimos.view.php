...
if(!empty($_POST['busca']) && $_POST['filtro'] == "titulo") {
    $sql = "SELECT * FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario AND emprestimo.situacao AND livro.titulo like :busca LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
    $statement = $pdo->prepare($sql);
    $statement->bindValue(':busca', '%' . $busca . '%');
}
else if(!empty($_POST['busca']) && $_POST['filtro'] == "usuario") {
    $sql = "SELECT * FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario AND emprestimo.situacao AND usuario.nome like :busca LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
    $statement = $pdo->prepare($sql);
    $statement->bindValue(':busca', '%' . $busca . '%');
}
else{
    $sql = "SELECT * FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario ORDER BY emprestimo.situacao LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
    $statement = $pdo->prepare($sql);
}
$statement->execute();
...
if(!empty($_POST['busca']) && $_POST['filtro'] == "titulo") {
    $sqlContador = "SELECT COUNT(*) AS total_registros FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario AND livro.titulo like :busca";
    $statement = $pdo->prepare($sqlContador);
    $statement->bindValue(':busca', '%' . $busca . '%');
}
if(!empty($_POST['busca']) && $_POST['filtro'] == "usuario") {
    $sqlContador = "SELECT COUNT(*) AS total_registros FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario AND usuario.nome like :busca";
    $statement = $pdo->prepare($sqlContador);
    $statement->bindValue(':busca', '%' . $busca . '%');
}
else{
    $sqlContador = "SELECT COUNT(*) AS total_registros FROM emprestimo, exemplar, livro, usuario where emprestimo.idExemplar = exemplar.idExemplar AND exemplar.idLivro = livro.idLivro AND emprestimo.idUsuario = usuario.idUsuario";
    $statement = $pdo->prepare($sqlContador);
}
$statement->execute();
    ...
    <div class="row content">
        ...
        <div id="conteudoPrincipal" class="col-md-10 text-left">
            ...
            <div id="DivRegistroTabela" class="container col-md-12">
                        ...
                        <div class="container col-md-8">
                            <div class="form-row">
                                <div id="buscarRegistro" class='input-group'>
                                    <input id='busca' name="busca" type='text' class='form-control' placeholder='Buscar emprestimo no sistema...'>
                            ...
                            <div id="filtroBuscar" class="col-md-6">
                                <select class="form-control" id="filtro" name="filtro" required >
                                    ...
                                    <option value="usuario" name="usuario">Buscar por Usuário</option>
                                </select>
                            </div>
                        </div>
                    </form>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <h1>Lista de Emprestimos no Sistema</h1>
                        <hr id="hrRegistros">
                        ...
                        <div id="tabelaExibir" class="row">
                            <table class="table table-hover">
                                ...
                                foreach ($dados as $emprestimo) {
                                    if($emprestimo->capa == null || $emprestimo->capa == ""){
                                        echo "
                                        <tr>                        
                                        ...
                                        echo "
                                        <tr>                        
                                        <td><img src='../imagens/livros/".$emprestimo->idLivro."/".$emprestimo->capa. "' width='60px'></td>";
                                    }

                                    echo "
                        <td>$emprestimo->titulo</td>
                        <td>$emprestimo->idExemplar</td>
                        ...
                        <td><a  data-toggle='modal' data-target='#myModal".$emprestimo->idEmprestimo."' id='tdRemover' href='#?id=".$emprestimo->idEmprestimo."'><i class='glyphicon glyphicon-remove'></i>  Devolução</a></td>
                    </tr>
                    <!-- Modal -->
                    <div id='myModal".$emprestimo->idEmprestimo."' class='modal fade' role='dialog'>
                        <div class='modal-dialog'>
                            ...
                            <div class='modal-content'>
                                ...
                                <div id='bodyModal' class='modal-body'>
                                    <p>Confirmar a devolução de emprestimo ?</p>
                                </div>
                                <div id='rodapeModal' class='modal-footer'>
                                    <a id='aBotaoRemover' class='btn btn-warning' href='../controller/Emprestimo.action.php?act=dev&id=".$emprestimo->idEmprestimo."'>Devolver</a>
